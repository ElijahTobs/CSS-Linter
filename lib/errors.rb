...
module ErrorsModule
  def space_before_semicolon?(chars, chars_index)
    ...
    puts 'Error'.red + ' on'.blue + " line:[:#{chars_index}:]".yellow + ' Unexpected space before semicolon ";"'
  end
  ...
  def indentation?(chars, chars_index)
    ...
    puts 'Error'.red + ' on'.blue + " line:[:#{chars_index}:]".yellow + ' Expecting indentation to be two spaced'
  ...
  def trailing_white_space?(chars, chars_index)
    if !chars.match?(/(?<=\S)[\t ]+$/) || chars.match?(/^\n$/)
      ...
      puts 'Error'.red + ' on'.blue + " line:[:#{chars_index}:]".yellow + ' Unexpected white space'
    end
  end
  ...
  def ending_semicolon?(chars, chars_index)
    if chars.match?(/(;.*)$/) || chars.match?(/({|})/) || chars.match?(/^\n$/)
      ...
      puts 'Error'.red + ' on'.blue + " line:[:#{chars_index}:]".yellow + ' Missing ending semicolon ";" '
    end
  end
  ...
  def space_before_colon?(chars, chars_index)
    ...
    puts 'Error'.red + ' on'.blue + " line:[:#{chars_index}:]".yellow + ' Unexpexted space before colon ":"'
  end
