...
module ErrorsModule
  ...
  def indentation?(_char, char_index)
    return true if char.match?(/^(\s{2})+([a-zA-Z0-9\-_]+)/)

    puts 'Error'.red + ' on' + " line:[:#{char_index}:]".yellow + ' Expecting indentation to be two spaced'
  end
  ...
  def trailing_white_space?(char, char_index)
    if !char.match?(/(?<=\S)[\t ]+$/) || char.match?(/^\n$/)
      ...
      puts 'Error'.red + ' on' + " line:[:#{char_index}:]".yellow + ' Unexpected white space'
    end
  end
